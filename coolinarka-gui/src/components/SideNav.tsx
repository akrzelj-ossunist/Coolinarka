...
const SideNav: React.FC<{ sx?: string; navContent?: any[] }> = ({
  ...
  return (
    <div
      className={`${sx} h-[100vh] bg-white shadow-2xl z-10 flex py-11 flex-col overflow-y-auto`}>
      {navContent?.map((content, index) => {
        return (
          <div key={index} className="flex items-center">
            {content.title === "Logo" ? (
              <p className="mb-20 flex justify-center items-center w-full">
                {content.logo}
              </p>
            ) : content.title === "Settings" ? (
              <div
                onClick={() => setDropMenu(dropMenu ? false : true)}
                className={`flex items-center my-4 w-full cursor-pointer relative ml-12`}>
                {content.logo}
              ...
              <Link
                to={`/${content.title}`}
                className={`flex items-center my-4 w-full cursor-pointer ease-in-out duration-300 ml-12 ${
